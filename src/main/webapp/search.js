...
prepositionInput.addEventListener('keydown', function(event) {
	 let isValueIncluded = false;
        ...
        for (const listItem of prepositions) {
            if (listItem.textContent ===  prepositionList.querySelector('li:not(.hide)').textContent)
             {
    ...
    if (event.key === 'Enter' && prepositionList.querySelector('li:not(.hide)')) {
        ...
        prepositionList.classList.add('hide');
        window.location.href = "searchServlet?depart="+prepositionInput.value;
       
    }else if(event.key === 'Enter' && !isValueIncluded) 
     { console.log("me famech");
		 //window.location.href = "searchServlet?depart="+"null"; 
	 }
